...
class _TimesheetListState extends State<TimesheetList> {
                            ...
                            children: [
                              ...
                              Table(
                                children: [
                                  ...
                                  buildContentTwoByTwoSubTitle(context,contentTitle1: item.addedDate!,contentTitle2: item.status!,fontSize: 14.0),
  ...
  Widget buildTimesheetList(TimesheetListData item,TimesheetFormController cont) {
    return  Card(
      color: faintGrey,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(7.0),
          side: const BorderSide(color: grey)),
      child: ExpansionTile(
        title:  buildTextRegularWidget("${item.firmEmployeeName!} on ${item.addedDateToShow!} ${item.timeHours!} hours "
        ...
        children: [
          Table(
            children: [
              ...
              buildContentTwoByTwoSubTitle(context,contentTitle1: item.inTime!,contentTitle2: item.outTime!,fontSize: 14.0),
          ...
          buildTextBoldWidget(item.status == "2" ? "Saved " :
                              item.status == "4" ? "Approved" :
                              item.status == "5" ? "Sent for Resubmission" : "Submit for Approval",
              blackColor, context, 14.0),
          const SizedBox(height: 15.0,),
          Row(
            children: [
              item.status == "Approved" ? const Opacity(opacity: 0.0,):
              ...
              Flexible(child: GestureDetector(onTap:(){
                cont.callTimesheetEdit(item.addedBy!,item.tDate!,"form");
              },
              ...
              Flexible(child: GestureDetector(
                  onTap: (){
                    cont.callTimesheetLog(context,item.tDate!,item.id!);
                  },
                  child:buildActionForClaim(buttonColor,Icons.view_stream_rounded)
              )),
              item.status == "Approved" ? const Opacity(opacity: 0.0,):
              const SizedBox(width: 5.0,),
              ...
              Flexible(child: GestureDetector(onTap:(){
                cont.callTimesheetEdit(item.addedBy!,item.tDate!,"view");
              },
