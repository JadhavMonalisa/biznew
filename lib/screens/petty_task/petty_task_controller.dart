...
class PettyTaskController extends GetxController {
  ...
  checkEstimatedHoursValidation(BuildContext context) {
    if (estimatedHours.text.isEmpty) {
      validateEstimatedHr = true;
      update();
    } else {
      validateEstimatedHr = false;
      update();
    }
  }
  ...
  checkEstimatedMinutesValidation(BuildContext context) {
    if (estimatedMinutes.text.isEmpty) {
      validateEstimatedM = true;
      update();
    } else {
      validateEstimatedM = false;
      update();
    }
  }
  ...
  checkValidation(BuildContext context) {
    ...
    if (selectedBranchName == "" ||
        ...
        selectedTriggerDate == "" ||
        selectedTargetDate == "" ||
        estimatedHours.text.isEmpty ||
        estimatedMinutes.text.isEmpty ||
        task.text.isEmpty ||
        fees.text.isEmpty ||
        estimatedHours.text.isEmpty ||
        estimatedMinutes.text.isEmpty) {
  ...
  Future<void> selectDate(BuildContext context, String dateFor) async {
    final DateTime? picked = await showDatePicker(
        ...
        firstDate:
            dateFor == "triggerDate" ? DateTime.now() : DateTime(1700, 1),
  ...
  void callClientList() async {
    ...
    try {
      BranchClientListModel? response =
          (await repository.getBranchClientList(selectedBranchId));
      if (response.success!) {
  ...
  void callEmpList() async {
    ...
    try {
      BranchEmpModel? response =
          (await repository.getBranchEmpList(selectedBranchId));
      if (response.success!) {
  ...
  void callAddPettyTask(BuildContext context) async {
    try {
      ApiResponse? response = (await repository.getAddPettyTask(
          selectedBranchId,
          selectedClientId,
          selectedEmpId,
  ...
  checkConstitutionValidation(String val) {
    if (constitutionList.isNotEmpty) {
      ...
      if (selectedConstitution == "") {
        addClientValidateConstitution = true;
        update();
      } else {
        addClientValidateConstitution = false;
        update();
      }
    }
  }
  ...
  checkClientCodeValidation(BuildContext context) {
    if (clientCode.text == "") {
      addClientValidateClientCode = true;
      update();
    } else {
      addClientValidateClientCode = false;
      update();
    }
  }
  ...
  checkCompanyNameValidation(BuildContext context) {
    if (companyName.text == "") {
      addClientValidateCompanyName = true;
      update();
    } else {
      addClientValidateCompanyName = false;
      update();
    }
  }
  ...
  checkClientGroupValidation(String val) {
    if (groupList.isNotEmpty) {
      selectedClientGroupName = val;
      update();
      if (selectedClientGroupName == "") {
        validateClientGroup = true;
        update();
      } else {
        validateClientGroup = false;
        update();
      }
    }
  }
  ...
  checkParentNameValidation(BuildContext context) {
    if (parentCompanyName.text == "") {
      validateParentCompany = true;
      update();
    } else {
      validateParentCompany = false;
      update();
    }
  }
  ...
  checkErstwhileValidation(BuildContext context) {
    if (erstWhileName.text == "") {
      validateErstwhileName = true;
      update();
    } else {
      validateErstwhileName = false;
      update();
    }
  }
  ...
  checkClientCinValidation(BuildContext context) {
    if (clientCin.text == "") {
      validateClientCin = true;
      update();
    } else {
      validateClientCin = false;
      update();
    }
  }
  ...
  checkClientPanValidation(BuildContext context) {
    if (clientPan.text == "") {
      validateClientPan = true;
      update();
    } else {
      validateClientPan = false;
      update();
    }
  }
  ...
  checkClientTanValidation(BuildContext context) {
    if (clientTan.text == "") {
      validateClientTan = true;
      update();
    } else {
      validateClientTan = false;
      update();
    }
  }
  ...
  checkClientGstinValidation(BuildContext context) {
    if (clientGstin.text == "") {
      validateClientGstin = true;
      update();
    } else {
      validateClientGstin = false;
      update();
    }
  }
  ...
  checkBusinessNatureValidation(String val) {
    if (businessNatureList.isNotEmpty) {
      ...
      if (selectedBusinessNature == "") {
        validateBusinessNature = true;
        update();
      } else {
        validateBusinessNature = false;
        update();
      }
    }
  }
  ...
  checkLandlineNoValidation(BuildContext context) {
    if (landlineNo.text == "") {
      validateLandlineNo = true;
      update();
    } else {
      validateLandlineNo = false;
      update();
    }
  }
  ...
  checkNoOfPlantsValidation(BuildContext context) {
    if (noOfPlants.text == "") {
      validateNoOfPlants = true;
      update();
    } else {
      validateNoOfPlants = false;
      update();
    }
  }
  ...
  checkWhatsAppNoValidation(BuildContext context) {
    if (whatsAppNo.text == "") {
      validateWhatsAppNo = true;
      update();
    } else {
      validateWhatsAppNo = false;
      update();
    }
  }
